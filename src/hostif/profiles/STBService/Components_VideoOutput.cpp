...
#define POWER_STATE_STRING "TvPowerState"
...
int hostIf_STBServiceVideoOutput::handleGetMsg(const char *paramName, HOSTIF_MsgData_t *stMsgData)
{
    ...
    else if(strcasecmp(paramName, POWER_STATE_STRING) == 0)
	{
    	ret = getTVPowerState(stMsgData);
	}
    return ret;
}
...
int hostIf_STBServiceVideoOutput::getTVPowerState(HOSTIF_MsgData_t *stMsgData,bool *pChanged)
{
	try {
		...
		*/
		...
		if (true == vPort.isDisplayConnected())
		{
			...
			if(vPort.isEnabled())
			{
				...
				if (isActive)
					strncpy(stMsgData->paramValue, ON_STRING, PARAM_LEN);
				else
					strncpy(stMsgData->paramValue, OFF_STRING, PARAM_LEN);
			}
			else
			{
				strncpy(stMsgData->paramValue, UNKNOWN_STRING, PARAM_LEN);
			}
		}
		else
		{
			RDK_LOG(RDK_LOG_ERROR,LOG_TR69HOSTIF,"[%s] HDMI Display is NOT connected, not able to retrieve TV power status\r\n",__FUNCTION__);
			strncpy(stMsgData->paramValue, UNKNOWN_STRING, PARAM_LEN);
		}
		stMsgData->paramValue[PARAM_LEN-1] = '\0';
		stMsgData->paramtype = hostIf_StringType;
		stMsgData->paramLen = strlen(stMsgData->paramValue);
		RDK_LOG(RDK_LOG_DEBUG,LOG_TR69HOSTIF,"[%s] TV Power Status is [%s]\r\n",__FUNCTION__,stMsgData->paramValue);
	}
    ...
    try {
        ...
        */
        if (true == vPort.isDisplayConnected())
        {
            ...
            if(vPort.isEnabled())
            {
                isHDCPSupported = (vPort.getHDCPStatus() == dsHDCP_STATUS_AUTHENTICATED)?true:false;
                if (isHDCPSupported)
